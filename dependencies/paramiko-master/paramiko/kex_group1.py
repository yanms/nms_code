...
class KexGroup1(object):
    ...
    def start_kex(self):
        ...
        m = Message()
        m.add_byte(pack_byte(_MSG_KEXDH_INIT))
        m.add_mpint(self.e)
    ...
    def _generate_x(self):
        ...
        while 1:
            x_bytes = bytearray(self.transport.rng.read(128))
            x_bytes[0] &= 0x7f
            #x_bytes = self.transport.rng.read(128)
            #x_bytes = chr(x_bytes[0] & 0x7f).encode() + x_bytes[1:]
    ...
    def _parse_kexdh_init(self, m):
        ...
        # send reply
        m = Message()
        m.add_byte(pack_byte(_MSG_KEXDH_REPLY))
        m.add_string(key)
        m.add_mpint(self.f)
