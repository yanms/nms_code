	...
	if qtype == 'models' and request.user.has_perm('nms.add_devices'):
		...
		if query == 'receive':
			...
			if type(text) != type(bytes()):
				History.objects.create(user_id = request.user, action_type='Manage device', dev_id = device, action = '[dev%i] %s' % (device.dev_id, text))
			...
			if type(text) != type(bytes()):
				History.objects.create(user_id = request.user, action_type='Elevate user (manage device)', dev_id = device, action = 'Password' % (device.dev_id, text))
			connection.send(text.encode())
			return HttpResponse('', content_type='text/plain')
